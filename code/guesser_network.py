...
class GuesserNetwork(AbstractNetwork):
    def __init__(self, config, num_words, device='', reuse=False,policy_gradient=False):
        ...
        with tf.variable_scope(self.scope_name, reuse=reuse):
            ...
            self.object_cats_emb = utils.get_embedding(
                ...
                config['cat_emb_dim'],
                scope='cat_embedding')
            self.objects_input = tf.concat([self.object_cats_emb, self.obj_spats], axis=2)# (mini_batch_size,max_objs,8+256)
            self.flat_objects_inp = tf.reshape(self.objects_input, [-1, config['cat_emb_dim'] + config['spat_dim']])#(mini_batch_size*max_objs, 8 + 256)
            with tf.variable_scope('obj_mlp'):
                h1 = utils.fully_connected(
                    ...
                    activation='tanh',
                    scope='l1')
                #h1 (mini_batch_size*max_objs, 512)
                ...
                h2 = utils.fully_connected(
                    ...
                    activation='tanh',
                    scope='l2')
                #h2 (mini_batch_size*max_objs, 512)
            self.obj_embs = tf.reshape(h2, [-1, tf.shape(self.obj_cats)[1], config['dialog_emb_dim']])#(batch_size, max_objs, 512)
            ...
            self.softmax,progressive_loss = self.VDST()#(batch_size, max_objs)

            self.selected_object = tf.argmax(self.softmax, axis=1)#row, (mini_batch_size,)

            #self.targets (mini_batch_size,)
            self.cross_entropy_loss = utils.cross_entropy(self.softmax, self.targets)#(mini_batch_size,)
            self.loss = 0.7*tf.reduce_mean(self.cross_entropy_loss) + 0.3*tf.reduce_mean(progressive_loss)
    ...
    def VDST(self):
        batch_size = tf.shape(self.dialog_3d_trans)[1]
        max_qlen = tf.shape(self.dialog_3d_trans)[-1]

        def cond(j,prev_p,c,h,p_subtraction):return tf.less(j,self.max_qnum)
        def body(j,prev_p,c,h,p_subtraction):
            ...
            attention = tf.reduce_sum(tf.multiply(self.obj_embs,tf.expand_dims(prev_p,-1)), axis=1, keep_dims=False)#(batch_size, 512)
            ...
            qa_j = tf.concat([state_h,aj_emb],axis=1)#(batch_size, 512+512)
            curr_p = self.CMM_Simplified(qa_j,prev_p,reuse=True)#(batch_size, max_objs)
            p_subtraction += utils.cross_entropy(1.1+curr_p-prev_p,self.targets)
            return tf.add(j, 1),curr_p,state_c,state_h,p_subtraction
        ...
        attention = tf.reduce_sum(tf.multiply(self.obj_embs,tf.expand_dims(init_prob,-1)), axis=1, keep_dims=False)#(batch_size, 512)
        ...
        #c,h (batch_size, 512)
        qa_1 = tf.concat([h,a1_emb],axis=1)#(batch_size, 512+512)
        curr_p = self.CMM_Simplified(qa_1,init_prob,reuse=False)#(batch_size, max_objs)
        p_subtraction = utils.cross_entropy(1.1+curr_p-init_prob,self.targets)#(batch_size,)

        j = tf.constant(1)
        _,facts,_,_,loss_subtraction = tf.while_loop(cond, body,
                    [j,curr_p,c,h,p_subtraction],
                    shape_invariants=[\
                        j.get_shape(),\
                        curr_p.get_shape(),\
                        c.get_shape(),\
                        h.get_shape(),p_subtraction.get_shape()])
        #self.facts (batch_size, max_objs)
        return facts,loss_subtraction
    ...
    def CMM_Simplified(self,h,prev_p,scope_name="CMM",reuse=False,weight_initializer=tf.contrib.layers.xavier_initializer()):
        ...
        with tf.variable_scope(scope_name, reuse=reuse):
            ...
            #CMM-version 1
            #U = tf.get_variable("CMM_U",[object_feature_size,scale_emb_size],initializer=weight_initializer)
            #Uo = tf.tanh(tf.einsum('ijk,kl->ijl',curr_obj_embs,U))#(batch_size, max_objs, scale_emb_size)
            #Vh = utils.fully_connected(h,scale_emb_size,use_bias=False,activation='tanh',reuse=reuse,scope="CMM_Vh")#(batch_size,scale_emb_size)    
            #Uo_odot_Vh = tf.multiply(tf.expand_dims(Vh,axis=1),Uo)#(batch_size, max_objs, scale_emb_size)
            #Uo_odot_Vh_reshape = tf.reshape(Uo_odot_Vh,[-1,scale_emb_size])#(batch_size * max_objs, scale_emb_size)
            #Uo_odot_Vh_MLP = utils.fully_connected(Uo_odot_Vh_reshape,1,use_bias=True,reuse=reuse,scope="CMM_Nonlinear")#(batch_size * max_objs,1)
            #Uo_odot_Vh_MLP = tf.reshape(Uo_odot_Vh_MLP,[batch_size,max_objs])
            #logits = tf.rsqrt(1.0*scale_emb_size)*Uo_odot_Vh_MLP #(batch_size, max_objs)

            #CMM version 2
            """
            odot_Obj_h = tf.multiply(tf.expand_dims(h,axis=1),curr_obj_embs)#(batch_size,max_objs,512)
            concat_odot_h_obj = tf.concat([odot_Obj_h,tf.tile(tf.expand_dims(h,axis=1),[1,max_objs,1]),curr_obj_embs],axis=2)#(batch_size,max_objs,512+512+512)
            concat_odot_h_obj_reshape = tf.reshape(concat_odot_h_obj,[-1,object_feature_size*3])#(batch_size*max_objs, 512)
            #odot_Obj_h_drop = tf.cond(self.is_training,lambda:tf.nn.dropout(odot_Obj_h,keep_prob=self.config['keep_prob']),lambda:odot_Obj_h)
            lay1 = utils.fully_connected(concat_odot_h_obj_reshape,512,use_bias=True,activation='tanh',reuse=reuse,scope="CMM_layer_1")
            #lay1_drop = tf.cond(self.is_training,lambda:tf.nn.dropout(lay1,keep_prob=self.config['keep_prob']),lambda:lay1)
            lay2 = utils.fully_connected(lay1,128,use_bias=True,activation='tanh',reuse=reuse,scope="CMM_layer_2")
            lay3 = utils.fully_connected(lay2,1,use_bias=True,reuse=reuse,activation='tanh',scope="CMM_layer_3")#(batch_size * max_objs,1)
            logits = tf.reshape(lay3,[batch_size,max_objs])
            #logits = tf.rsqrt(1.0*scale_emb_size)*lay2_reshape #(batch_size, max_objs)
            """
            
            #CMM version 3
            Vh = utils.fully_connected(h,512,use_bias=False,activation='tanh',reuse=reuse,scope="CMM_Vh")#(batch_siz,512)
            ...
            p_norm = self.obj_mask*tf.div(accum_p,tf.reduce_sum(accum_p, axis=1,keepdims=True))#(batch_size, max_objs)
            return p_norm

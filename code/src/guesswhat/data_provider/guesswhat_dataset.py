...
"""
Dataset(args.data_dir, "train", image_loader, crop_loader)
data/guesswhat.test.jsonl.gz 37M
data/guesswhat.train.jsonl.gz 174M
data/guesswhat.valid.jsonl.gz 36M
"""
class Dataset(AbstractDataset):
    """Loads the dataset."""
    def __init__(self, folder, which_set, image_builder=None, crop_builder=None,used_num=None):
        ...
        with gzip.open(file) as f:
            for line in f:
                ...
                g = Game(id=game['id'],
                         ...
                         crop_builder=crop_builder)
                games.append(g)
                #print("{}_pw_img_id\t{}".format(which_set,g.image.id))
                if used_num is not None and len(games) >= used_num:break
...
def dump_samples_into_dataset(data, save_path, tokenizer, name="model"):
    ...
    with gzip.open(save_path.format('guesswhat.' + name + '.jsonl.gz'), 'wb') as f:
        for id_game, d in enumerate(data):
            ...
            success = d["success"]
            prob_objects = d["prob_objects"]#[question_nums, object_nums]

...
class GuesserWrapper(object):
    ...
    """
    def find_object(self, sess,dialogues, seq_length, game_data,dialog_3d,dialog_3d_answer,dialog_question_num,max_qnum):
        game_data["dialogues"] = dialogues
        game_data["seq_length"] = seq_length
        #==============add==========
        game_data["dialog_3d"] = dialog_3d
        game_data["dialog_3d_answer"] = dialog_3d_answer
        game_data["dialog_question_num"] = dialog_question_num
        game_data["max_qnum"] = max_qnum

        #sample
        selected_object, softmax,predicted_sequence = self.evaluator.execute(sess, output=[self.guesser.selected_object, self.guesser.softmax,self.guesser.predicted_sequence], batch=game_data)

        found = (selected_object == game_data["targets_index"])

        return found, softmax, selected_object,predicted_sequence

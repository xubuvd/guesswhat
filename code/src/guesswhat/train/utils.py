...
def compute_qgen_accuracy(sess, dataset, batchifier, evaluator, mode, tokenizer, save_path, cpu_pool, batch_size, store_games, dump_suffix,log_level=2):
    ...
    for m in mode:
        test_iterator = Iterator(dataset, pool=cpu_pool,
                                 ...
                                 shuffle=False,
                                 use_padding=True)
        test_score = evaluator.process(sess, test_iterator, mode=m, store_games=store_games, log_level=log_level)
        ...
        logger.info("ErroRate ({} - {}): {}".format(dataset.set, m, 1.0-test_score))

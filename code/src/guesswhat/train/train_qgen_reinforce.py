...
if __name__ == '__main__':
    ...
    parser.add_argument("-exp_guesser_RL_dir", type=str, required=True, help="Directory in which experiments are stored")
    parser.add_argument("-img_dir", type=str, help='Directory with images')
    parser.add_argument("-crop_dir", type=str, help='Directory with images')
    parser.add_argument("-config", type=str, required=True, help='Config file')
    parser.add_argument("-guesser_RL_config", type=str, required=True, help='Config file')
    ...
    guesser_RLConfig,_,guesser_RL_save_path = load_config(args.guesser_RL_config,args.exp_guesser_RL_dir)
    #copy code
    ...
    os.system(cmd)
    cmd="cp -rf /home/pw/guesswhat/Train_Looper.sh {}".format(code_path)
    os.system(cmd)
    # Load all  networks configs
    oracle_config,oracle_save_path = get_config_from_xp(os.path.join(args.networks_dir, "oracle"), args.oracle_identifier)
    guesser_config,guesser_save_path = get_config_from_xp(os.path.join(args.networks_dir, "guesser"), args.guesser_identifier)
    ...
    guesser_network = GuesserNetwork(guesser_config["model"], num_words=tokenizer.no_words,policy_gradient=True)
    ...
    pg_variables_guesser = [v for v in tf.trainable_variables() if "guesser" in v.name and 'rl_baseline' not in v.name]
    baseline_variables_guesser = [v for v in tf.trainable_variables() if "guesser" in v.name and 'rl_baseline' in v.name]
    print("pg_variables_guesser:{}".format(pg_variables_guesser))
    print("baseline_variables_guesser:{}".format(baseline_variables_guesser))
    ...
    pg_global_steps = tf.Variable(-1, trainable=False)
    pg_optimize, pgloss,pg_lr = create_optimizer(qgen_network, loop_config,
                                   var_list=pg_variables,
                                   optim_cst=tf.train.MomentumOptimizer,
                                   loss=qgen_network.policy_gradient_loss,staircase=True,global_step=pg_global_steps,momentum=True)

    bsglobal_steps = tf.Variable(-1, trainable=False)
    baseline_optimize, baseloss,bs_lr = create_optimizer(qgen_network, loop_config,
                                        var_list=baseline_variables,
                                        optim_cst=tf.train.MomentumOptimizer,
                                        apply_update_ops=False,
                                        loss=qgen_network.baseline_loss,staircase=True,global_step=bsglobal_steps,momentum=True)
    optimizer = [pg_optimize, baseline_optimize]

    g_global_steps = tf.Variable(-1, trainable=False)
    # post-train Guesser using Policy Gradient Algorithm
    guesser_optimize, guesser_loss,g_lr = create_optimizer(guesser_network,guesser_RLConfig,
                                    var_list=pg_variables_guesser,
                                    optim_cst=tf.train.MomentumOptimizer,
                                    loss=guesser_network.policy_gradient_loss,staircase=True,global_step=g_global_steps,momentum=True)
    g_optimizer = [guesser_optimize]
    ...
    # Load config
    mode_to_evaluate = ["greedy","sampling"]
    ...
    with tf.Session(config=cfg) as sess:
        ...
        #    print("NodeName:{}".format(node))
        sess.run(tf.global_variables_initializer())
        if args.continue_exp: # TODO only reload qgen ckpt
            qgen_saver.restore(sess, save_path.format('params.ckpt'))
            guesser_saver.restore(sess, guesser_RL_save_path.format('params.ckpt'))
        else:
            qgen_var_supervized = [v for v in tf.global_variables() if "qgen" in v.name and 'rl_baseline' not in v.name and "Momentum" not in v.name] 
            ...
            guesser_var_supervized = [v for v in tf.global_variables() if "guesser" in v.name and 'rl_baseline' not in v.name and "Momentum" not in v.name]
            guesser_loader_supervized = tf.train.Saver(var_list=guesser_var_supervized)
            guesser_loader_supervized.restore(sess, os.path.join(args.networks_dir, 'guesser', args.guesser_identifier, 'params.ckpt'))

        start_epoch,final_val_score = load_checkpoint(sess, loop_saver, args, save_path,default=0.)
        ...
        compute_qgen_accuracy(sess, trainset, batchifier=train_batchifier, evaluator=looper_evaluator, tokenizer=tokenizer,
                              ...
                              store_games=args.store_games, dump_suffix="init.new_object",log_level=0)
        ...
        compute_qgen_accuracy(sess, testset, batchifier=eval_batchifier, evaluator=looper_evaluator, tokenizer=tokenizer,
                              mode=mode_to_evaluate, save_path=save_path, cpu_pool=cpu_pool, batch_size=batch_size,
                              store_games=args.store_games, dump_suffix="init.new_games",log_level=0)
        logger.info(">>>------------------------------------------------<<<")
        ...
        for epoch in range(start_epoch,no_epoch):
            ...
            logger.info("Epoch {}/{}".format(epoch+1, no_epoch))
            ...
            train_score = looper_evaluator.process(sess, train_iterator,
                                                   optimizer=optimizer,
                                                   mode="sampling",epoch=epoch+1,log_level=log_level,G_optimizer=g_optimizer)
            logger.info("pg_global_steps:{}\tbsglobal_steps:{}\tg_global_steps:{}".format(sess.run(pg_global_steps),sess.run(bsglobal_steps),sess.run(g_global_steps)))
            logger.info("pg_lr:{}\tbs_lr:{}\tg_lr:{}".format(sess.run(pg_lr),sess.run(bs_lr),sess.run(g_lr)))
            #logger.info("pg_momentum_exp:{}\tg_momentum_exp:{}".format(sess.run(pg_momentum_exp),sess.run(g_momentum_exp)))
            ...
            if val_score > final_val_score:
                ...
                pickle_dump({'epoch': epoch,'best_val_loss':final_val_score}, save_path.format('status.pkl'))
                guesser_saver.save(sess,guesser_RL_save_path.format('params.ckpt'))
        ...
        compute_qgen_accuracy(sess, trainset, batchifier=train_batchifier, evaluator=looper_evaluator, tokenizer=tokenizer,
                              mode=mode_to_evaluate, save_path=save_path, cpu_pool=cpu_pool, batch_size=batch_size,
                              store_games=args.store_games, dump_suffix="final.new_object",log_level=log_level)

        logger.info(">>>  New Games  <<<")
        compute_qgen_accuracy(sess, testset, batchifier=eval_batchifier, evaluator=looper_evaluator, tokenizer=tokenizer,
                              mode=mode_to_evaluate, save_path=save_path, cpu_pool=cpu_pool, batch_size=batch_size,
                              store_games=args.store_games, dump_suffix="final.new_games",log_level=log_level)
        logger.info(">>>------------------------------------------------<<<")
